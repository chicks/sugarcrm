...
module SugarCRM; class Session
  ...
  def self.from_file(path, opts={})
    ...
    self.from_hash(config_values, opts)
  end

  # Combine existing config files and connect with resulting hash
  ...
  def self.parse_config_file(path)
    ...
    contents = self.symbolize_keys(contents)
  ...
  def load_config_files
    # see README for reasoning behind the priorization
    self.class.config_file_paths.each{|path|
