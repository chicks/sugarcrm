...
* add your credentials to `config/config.yaml` (will need to be copied each time you upgrade or reinstall the gem)
...
An example, accompanied by instructions, can be found in the `config/config.yaml` file. In addition, a working example used for testing can be found in `test/config_test.yaml`
...
3.  * if your login credentials are stored in the `config/config.yaml` file, you can connect with `SugarCRM.connect!`
    ...
    * if you don't have a configuration file, you can always call the basic `SugarCRM.connect` and give it the proper arguments (see documentation above)

4. Once connected, you have full access to the gem's funcitonality, e.g. `puts SugarCRM::Account.first.name`
  
== EXPANDING THE GEM

If you want to expand the gem's capabilities with monkey patches (e.g. to add methods specific to your environment), you can either

* drop your `*.rb` files in `lib/sugarcrm/monkey_patches/` (see the README in that folder)

* drop your `*.rb` files in any other folder and call `SugarCRM::Environment.monkey_patch_folder = ` followed by the absolute path to the folder containing your monkey patches
