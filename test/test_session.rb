...
class TestSession < ActiveSupport::TestCase
  context "The SugarCRM::Session class" do
    ...
    should "parse config parameters from a file" do
      assert_equal CONFIG_CONTENTS[:config], SugarCRM::Session.parse_config_file(CONFIG_TEST_PATH)
    end
    ...
    should "use a given config scope" do
      config = {
        :configuration => {
          :production => {
            :username => 'sugarcrm',
            :password => 'default',
            :base_url => 'http://somesugarinstance.com'
          }
        }
      }
      SugarCRM::Session.config_scope = [:configuration, :production]
      assert_equal config[:configuration][:production], SugarCRM::Session.config_for_scope(config)
      SugarCRM::Session.config_scope = :config # reset to default
    end

    should "raise InvalidConfiguration if scope was not found" do
      SugarCRM::Session.config_scope = :wrong
      assert_raises SugarCRM::InvalidConfiguration do
        SugarCRM::Session.config_for_scope(CONFIG_TEST_PATH)
      end
      SugarCRM::Session.config_scope = :config # reset to default
    end
  end
